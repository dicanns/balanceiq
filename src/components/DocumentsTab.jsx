...
export default function DocumentsTab({ isFranchisor, orgId, cloudUser, T, t, onUnreadCountChange }) {
 ...
 style={{ padding:'4px 10px', borderRadius: 5, border: '1px solid rgba(255,255,255,0.2)', background: 'transparent', color: 'inherit', fontSize: 11, cursor: 'pointer' }}>
      ...
      {/* Documents section */}<div style={{ display: 'flex', gap: 16, minHeight: 300 }}>{/* Folder panel */}<div style={{ width: 180, flexShrink: 0, display: 'flex', flexDirection: 'column', gap: 4 }}><div style={{ fontSize: 11, fontWeight: 700, opacity: 0.5, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>{T.docFolders}</div>{folders.map(f => (<button key={f} onClick={() =>setSelectedFolder(f)} style={folderBtn(selectedFolder === f)}>
 {f}</button>))}
 {isFranchisor && (
 addingFolder
 ?<div style={{ display:'flex', gap: 4, marginTop: 4 }}><input value={newFolderName} onChange={e =>setNewFolderName(e.target.value)}
                    placeholder={T.docNewFolder} style={{ ...inputStyle, fontSize: 11 }}
                    onKeyDown={e => e.key === 'Enter' && handleAddFolder()} autoFocus /><button onClick={handleAddFolder} style={{ ...btnOrange, padding: '4px 8px' }}>+</button></div>:<button onClick={() =>setAddingFolder(true)} style={{ ...folderBtn(false), color: '#f97316', marginTop: 4 }}>
                  {T.docAddFolder}</button>)}</div>{/* File list */}<div style={{ flex: 1 }}>{!selectedFolder ? (<div style={{ fontSize: 12, opacity: 0.5, padding: '12px 0' }}>{T.docNoFolder}</div>) : (<><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}><div style={{ fontSize: 13, fontWeight: 700 }}>{selectedFolder}</div>{isFranchisor && (<div><input ref={fileInputRef} type="file" style={{ display: 'none' }} onChange={handleUpload}
                      accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png,.gif,.mp4,.mov,.zip" /><button onClick={() =>!uploading && fileInputRef.current?.click()} style={btnOrange} disabled={uploading}>
                      {uploading ? T.docUploading : T.docUpload}</button></div>)}</div>{uploadErr &&<div style={{ fontSize: 11, color: '#ef4444', marginBottom: 8 }}>{uploadErr}</div>}
              {filteredDocs.length === 0 ? (<div style={{ fontSize: 12, opacity: 0.5 }}>{T.docNoFiles}</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{filteredDocs.map(doc => (<div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', background: 'rgba(255,255,255,0.04)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.08)'}}><span style={{ fontSize: 18 }}></span><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, fontWeight: 600, overflow:'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{doc.filename}</div><div style={{ fontSize: 10, opacity: 0.5 }}>{doc.size_bytes ? T.docSize(doc.size_bytes) : ''} &nbsp;·&nbsp;
                          {new Date(doc.uploaded_at).toLocaleDateString()}</div></div><button onClick={() =>handleDownload(doc)} style={btnGhost}>{T.docDownload}</button>{isFranchisor && (<button onClick={() =>handleDelete(doc)} style={{ ...btnGhost, color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }}>{T.docDelete}</button>)}</div>))}</div>)}</>)}</div></div>{/* Announcements section */}<div><div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>{T.annSection}</div>{/* Compose (franchisor only) */}
        {isFranchisor && (<div style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.04)', borderRadius: 10, border: '1px solid rgba(255,255,255,0.1)', marginBottom: 14 }}><div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>{T.annNew}</div><input value={annTitle} onChange={e =>setAnnTitle(e.target.value)}
              placeholder={T.annTitleLabel} style={{ ...inputStyle, marginBottom: 6 }} /><textarea value={annBody} onChange={e =>setAnnBody(e.target.value)}
              placeholder={T.annBody} rows={3}
              style={{ ...inputStyle, resize: 'vertical', fontFamily: 'inherit' }} /><button onClick={handlePostAnn} disabled={annPosting || !annTitle.trim() || !annBody.trim()} style={{ ...btnOrange, marginTop: 8, opacity: (!annTitle.trim() || !annBody.trim()) ? 0.5 : 1 }}>{annPosting ? T.annPublishing : T.annPublish}</button></div>)}

        {/* Announcement list */}
        {announcements.length === 0 ? (<div style={{ fontSize: 12, opacity: 0.5 }}>{T.annNoAnn}</div>) : (<div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{announcements.map(ann => {
              ...
              return (<div key={ann.id}
                  ...
                  }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 13, fontWeight: 700 }}>{ann.title}</span>{isUnread &&<span style={{ fontSize: 9, fontWeight: 700, color: '#f97316', background: 'rgba(249,115,22,0.15)', border: '1px solid rgba(249,115,22,0.3)', borderRadius: 10, padding: '1px 6px' }}>{T.annUnread}</span>}</div><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 10, opacity: 0.4 }}>{new Date(ann.created_at).toLocaleDateString()}</span>{isFranchisor &&<button onClick={e =>{ e.stopPropagation(); handleDeleteAnn(ann); }} style={{ ...btnGhost, fontSize: 10, color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }}>{T.annDelete}</button>}</div></div><div style={{ fontSize: 12, opacity: 0.75, whiteSpace: 'pre-wrap' }}>{ann.body}</div></div>);
            })}</div>)}</div></div>
  );
}
