...
function StepType({ lang, companyInfo, saveCompanyInfo, onNext }) {
  ...
  return (<div><div style={STYLES.stepTitle}>{lang === 'fr' ? 'Quel type de restaurant?' : 'What type of restaurant?'}</div><div style={STYLES.stepDesc}>{lang === 'fr'
          ? 'Cela aide à pré-configurer vos catégories et étiquettes.'
          : 'This helps pre-configure categories and labels.'}</div><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>{RESTAURANT_TYPES.map((t) => {
          ...
          return (<button
            ...
            ><span style={{ fontSize: '22px', lineHeight: 1 }}>{t.icon}</span><span style={{ fontSize: '13px', fontWeight: '500', color: isSelected ? '#F97316' : '#E8E8EC', textAlign: 'left', lineHeight: '1.3' }}>{lang === 'fr' ? t.labelFr : t.labelEn}</span></button>);
        })}</div></div>);
}
...
function StepInfo({ lang, companyInfo, saveCompanyInfo, onNext, onBack }) {
  ...
  return (<div><div style={STYLES.stepTitle}>{lang === 'fr' ? 'Informations de base' : 'Basic information'}</div><div style={STYLES.stepDesc}>{lang === 'fr'
          ? 'Ces informations apparaîtront sur vos rapports et factures.'
          : 'This information will appear on your reports and invoices.'}</div><div style={STYLES.fieldGroup}><label style={STYLES.label}>{lang === 'fr' ? 'Nom du restaurant' : 'Restaurant name'}</label><input
          style={STYLES.input}
          type="text"
          value={nom}
          onChange={(e) =>setNom(e.target.value)}
          placeholder={lang === 'fr' ? 'Ex: Chez Martin' : 'e.g. Martin\'s Grill'}
        /></div><div style={STYLES.fieldGroup}><label style={STYLES.label}>{lang === 'fr' ? 'Ville' : 'City'}</label><input
          style={STYLES.input}
          type="text"
          value={ville}
        ...
        /></div><div style={STYLES.fieldGroup}><label style={STYLES.label}>{lang === 'fr' ? 'Langue de l\'interface' : 'Interface language'}</label><div style={{
          ...
          disabled={!nom.trim()}
        >{lang === 'fr' ? 'Continuer' : 'Continue'} →</button></div></div>);
}
...
function StepRegisters({ lang, roster, saveRoster, onNext, onBack }) {
  ...
  return (<div><div style={STYLES.stepTitle}>{lang === 'fr' ? 'Vos caisses' : 'Your registers'}</div><div style={STYLES.stepDesc}>{lang === 'fr'
          ? 'La plupart des restaurants ont 1–2 caisses.'
          : 'Most restaurants have 1–2 registers.'}</div><div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>{[1, 2, 3, 4, 5].map((n) => {
...
function StepDemo({ lang, onNext, onBack }) {
  return (<div><div style={STYLES.stepTitle}>{lang === 'fr' ? 'Votre premier jour équilibré' : 'Your first balanced day'}</div><div style={STYLES.stepDesc}>{lang === 'fr'
        ...
        marginBottom: '20px',
              ...
              borderRadius: '20px',
              padding: '4px 12px',
            }}><span style={{ fontSize: '12px', color: '#22C55E' }}></span><span style={{ fontSize: '12px', fontWeight: '600', color: '#22C55E' }}>{lang === 'fr' ? 'Balancé' : 'Balanced'}</span></div></div></div></div><div style={{
        ...
        lineHeight: '1.5',
        marginBottom: '4px',
      }}><span style={{ fontWeight: '600', display: 'block', marginBottom: '4px' }}>{lang === 'fr' ? 'Voilà!' : 'There you go!'}</span>{lang === 'fr'
          ? 'BalanceIQ signale automatiquement les écarts entre votre caisse et votre POS.'
          : 'BalanceIQ automatically flags discrepancies between your drawer and your POS.'}</div><div style={STYLES.footer}><button style={STYLES.btnSecondary} onClick={onBack}>← {lang === 'fr' ? 'Retour' : 'Back'}</button><button
          style={{ ...STYLES.btnPrimary, width: 'auto', flex: 1 }}
          onClick={onNext}
        >{lang === 'fr' ? 'Continuer' : 'Continue'} →</button></div></div>);
}
...
function StepReady({ lang, onBack, onComplete }) {
  ...
  return (<div><div style={{ textAlign: 'center', marginBottom: '8px' }}><div style={{ fontSize: '40px', marginBottom: '8px' }}></div><div style={STYLES.stepTitle}>{lang === 'fr' ? 'Vous êtes prêt!' : "You're ready!"}</div><div style={STYLES.stepDesc}>{lang === 'fr'
            ? 'Voici vos prochaines étapes suggérées :'
            : 'Here are your suggested next steps:'}</div></div><div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '24px' }}>{steps.map((s, i) => (<div
            key={i}
            style={{
              background: 'rgba(255,255,255,0.03)',
          ...
          ><div style={{
              ...
              fontSize: '16px',
              flexShrink: 0,
            }}>{s.icon}</div><div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: '13px', fontWeight: '500', color: '#E8E8EC', marginBottom: '2px' }}>{lang === 'fr' ? s.textFr : s.textEn}</div><div style={{ fontSize: '11px', color: '#6B7080', fontFamily: "'Satoshi', -apple-system, BlinkMacSystemFont, sans-serif" }}>{lang === 'fr' ? s.whereFr : s.whereEn}</div></div><div style={{ width: '18px', height: '18px', borderRadius: '50%', border: '2px solid rgba(255,255,255,0.1)', flexShrink: 0 }} /></div>))}</div><button
        style={{ ...STYLES.btnPrimary, fontSize: '15px', padding: '13px 20px' }}
        onClick={onComplete}
      >{lang === 'fr' ? ' Commencer' : ' Get started'}</button><div style={{ marginTop: '12px', textAlign: 'center' }}><button style={{ ...STYLES.btnSecondary, background: 'none', border: 'none', color: '#6B7080', fontSize: '12px' }} onClick={onBack}>← {lang === 'fr' ? 'Retour' : 'Back'}</button></div></div>);
}
